...
df <- df %>% 
  mutate(days_marriage_to_birth = 
         ...
         prenup_preg = ifelse(days_marriage_to_birth < 280, 1, 0))
...
### We can see a time trend

- More parents get married while the mother is pregnant after the 1910s

### But, it is difficult to see proportions

]

---
# How many pregnancies of each type do we have?
...
df %>% 
  ...
  summarise(n_obs = n()) %>%
  ungroup() %>% 
  # pivot_wider(names_from = prenup_preg, names_prefix = "preg", values_from = n_obs)
...
```{r, ages-1, eval=F}
df %>% 
  mutate(age_diff = female_age - male_age) %>% 
  ggplot(aes(age_diff)) +
  geom_density()


  # select(surname_b, male_age, female_age) %>% 
  # pivot_longer(-surname_b, names_to = "age", values_to = "value")
  # mutate(mean_male_age = mean(male_age, na.rm = T),
  #        mean_female_age = mean(female_age, na.rm = T)) %>% 
  # ggplot() +
  # geom_density(aes(male_age, fill = "Male age"), fill = "blue", alpha = .5) +
  # # geom_vline(xintercept = distinct(mean_male_age), )
  # geom_density(aes(female_age),  fill = "red", alpha = .5)
...
---
# Do these differ by type of pregnancy?
...
Or we can lump with `fct_lump` the remaining categories into "Other". 
...
# Return to your familiar situation having changed
